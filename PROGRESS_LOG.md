...
## Bloque 61 — Ciclo Aprobar/Descartar en la pestaña Automatización + carpeta de migraciones versionadas

Pedido de 4 partes sobre `automation_tasks` (el usuario ya había corrido el SQL — confirmado en esta misma sesión, la tabla existe y responde `0` filas): (1) rediseñar la gestión de tareas a tres opciones explícitas (Aprobar/Ejecutar, Descartar, sin marcar), (2) separar desarrollo local de despliegue a producción con automatización los viernes 8PM + botón manual, (3) protección de datos vía migraciones versionadas/aisladas. Este bloque resuelve los puntos 1 y 3; el punto 2 queda como una decisión de arquitectura que se le devuelve al usuario antes de tocar nada (ver el resto de la conversación) porque cambia un acuerdo explícito ya tomado (push a `main` = deploy inmediato, ver `feedback_autopush_policy` en memoria) y requiere un paso manual del usuario en el dashboard de Vercel al que Code no tiene acceso.

**1. Ciclo de vida de `automation_tasks` reescrito de pending→completed/failed a pending→approved/discarded**, calcando 1:1 el pedido explícito: "✅ Aprobar/Ejecutar" (`status: "approved"`), "❌ Descartar" (`status: "discarded"` — ARCHIVA, no borra la fila; un DELETE real disparado por un botón de UI es una acción difícil de revertir, así que se prefirió el mismo patrón "soft" que ya usa `feedback`/Inspector Agent para "descartado"), "sin marcar" es simplemente no tocar nada — la tarea se queda en `pending`, que ya es el default de la tabla, sin código nuevo necesario. Aclarado explícitamente en un comentario del HTML y en el código: "Aprobar" solo cambia el `status` en Supabase — **no dispara ningún ejecutor automático de código**, porque ese ejecutor no existe (ver `AUTOMATION_WORKFLOW.md`, sección "Por qué NO hay un ejecutor automático todavía"); es la señal de "listo para que un humano o una sesión de Claude Code lo tome", igual que ya funciona `approved_tasks.json` para el canal local.

**2. Bug real encontrado de paso: la pestaña mostraba `"Transacciones consolidadas: 0"` para tareas de automatización** — reutilizaba la clave de i18n `adminPanelRowCount` (pensada para la pestaña de Transacciones) sin adaptar el texto. Se agregó una clave dedicada `automationRowCount` ("Tareas encontradas:") — no se tocó `adminPanelRowCount` en sí ni la pestaña de Inspector de Bugs (que tiene el mismo problema pre-existente, fuera del alcance de este pedido).

**3. `supabase/migrations/` — carpeta nueva en la raíz del repo, con un `README.md` que fija la convención hacia adelante** (`YYYYMMDDHHMMSS_descripcion.sql`, un archivo por cambio de esquema, nunca editado después de correrse) y un primer archivo real: `20260801190000_automation_tasks.sql`, con el SQL EXACTO que el usuario corrió (no una versión idealizada) — incluye el aviso de seguridad sobre la política RLS abierta directo en el comentario del archivo, para que quede pegado al SQL que la causa, no solo en un documento aparte. Documentado con honestidad que esta carpeta NO es retroactiva al 100%: el SQL de `feedback`/`transactions` de bloques anteriores se entregó directo en el chat de esas sesiones, nunca quedó guardado en el repo, así que no se puede reconstruir con certeza total — si el usuario quiere el registro completo, tendría que exportarlo del historial del SQL Editor de Supabase.

**`sw.js`:** `CACHE_NAME` subido a `v20260801-38` (dos bumps intermedios, -37 y -38, durante la sesión de pruebas — el mismo problema de caché de sesiones anteriores: cada edición después de un `navigate({force:true})` necesita SU PROPIO bump para que el navegador de pruebas deje de servir el `<script>` viejo).
...
## Bloque 65 — Causa raíz real de "Algo falló" en Escuadrones + mensajes que no sincronizaban, Conversación como pestaña principal, y diagnóstico visible de la Base de Datos

Pedido: (1) mover "Conversación" (traductor cara a cara) a una pestaña principal, ya no anidada dentro de un chat individual; (2) que el micrófono capture/transcriba/traduzca de forma fluida; (3) arreglar el error "Algo falló — intenta de nuevo" al crear un Escuadrón; (4) que los mensajes a contactos se sincronicen en tiempo real.

**Diagnóstico real antes de tocar código (puntos 3 y 4):** consulté `https://miikaeru-web.vercel.app/api/init-db` directo en el navegador — responde `{"ok":false,"error":"Falta SUPABASE_DB_URL..."}`. Confirmé además contra la REST API de Supabase (anon key, sin pasar por el backend) que **ninguna tabla de la app existe en el proyecto real** (`app_contacts`, `app_friend_messages`, `player_progress`, las de Escuadrones del Bloque 64 — todas 404 "Could not find the table"). Causa raíz real: la variable de entorno `SUPABASE_DB_URL` nunca se configuró en Vercel, así que `/api/init-db` nunca corrió con éxito — no es un bug introducido por el Bloque 64, es que el backend de Amigos/Chat jamás llegó a aprovisionarse en producción. Esto explica ambos síntomas: Escuadrones no puede crear filas en una tabla que no existe, y los mensajes de Amigos nunca tuvieron ni tabla ni Realtime habilitado para sincronizar.

**Esto requiere una acción manual tuya que este código no puede hacer solo** (implica una contraseña de base de datos, un secreto que no debe pasar por acá): Supabase → Settings → Database → Connection string (modo "Transaction pooler", puerto 6543) → copiar → Vercel → Project Settings → Environment Variables → agregar `SUPABASE_DB_URL` → redeploy. En cuanto exista esa variable, la primera carga de la app ya desplegada dispara sola `/api/init-db` (mecanismo ya existente, ver `DB_INIT_FLAG_KEY` en `app.js`) y crea todo — Escuadrones y sincronización de mensajes quedan operativos sin ningún otro cambio de código.

**1. `api/init-db.js` robustecido — statements aislados.** Antes, `for (const statement of SCHEMA_STATEMENTS) { await client.query(statement); }` corría todo dentro de un único `try`: un solo statement que fallara abortaba el resto, incluidas tablas totalmente independientes. Ahora cada statement corre con su propio try/catch (`runStatement()`), se acumulan en `failedStatements`, y la respuesta JSON incluye el detalle exacto de qué falló y por qué (antes solo se veía el primer error y ahí se cortaba todo). También envolví `ensureRealtimeEnabled()`/`ensureFeedbackAdminPolicies()` en sus propios try/catch por el mismo motivo.

**2. Bug real de Realtime encontrado: faltaba `REPLICA IDENTITY FULL` en `app_friend_messages`.** Sin esto, un evento Realtime de `UPDATE` (que es justo lo que dispara `sendFriendMessage()` al llegar la traducción, después del INSERT inicial) solo trae en `payload.new` la primary key + las columnas que cambiaron — NO `phone_from`/`phone_to`, que es justo lo que `wireFriendRealtime()`/`friendConversationMatches()` usan para decidir si el evento es de la conversación abierta. Sin esas columnas, la traducción que llega por UPDATE nunca hacía match y el destinatario se quedaba viendo el mensaje sin traducir hasta refrescar a mano. Agregado `alter table app_friend_messages replica identity full` — statement idempotente, no rompe nada aunque se corra de nuevo.

**3. Diagnóstico visible: Panel de Administrador → 🗄️ Base de Datos (nueva pestaña).** Antes, un fallo de `/api/init-db` solo dejaba un `console.warn` — invisible para el Admin salvo que abriera devtools. `checkDatabaseStatus()` en `app.js` llama a `/api/init-db` a demanda (botón "Verificar / Reintentar", también se auto-ejecuta al abrir la pestaña) y muestra el JSON de respuesta completo (`ok`/`error`/`failedStatements`/`realtimeErrors`) en un `<pre>` — así confirmar que la variable de entorno ya quedó bien configurada no depende de F12.

**4. Mensaje honesto en Escuadrones.** `createSquad()`/`joinSquadByCode()` ya distinguían internamente `"schema-missing"` de otros errores; lo que faltaba era mapearlo a un texto propio en vez de caer en el genérico "Algo falló — intenta de nuevo" (que sonaba a que reintentar alcanzaba). Ahora muestra "El sistema de Escuadrones todavía se está configurando en el servidor — probá de nuevo en unos minutos" — honesto sobre qué pasa realmente, mismo criterio que ya usa "Amigos" con `chatFriendSchemaMissing`.

**5. Conversación como pestaña principal (punto 1 y 2 del pedido).** Se agregó `data-chat-tab="conversation"` a `#chat-tabs` (ahora 4 pestañas) y `#chat-conversation-mode` pasó de `data-chat-panel="conversation-mode"` a `data-chat-panel="conversation"` — con ese solo cambio, el mecanismo GENÉRICO de pestañas que ya alternaba Todos/Escuadrón/Amigos (`document.querySelectorAll(".chat-tab-panel")` en `app.js`) empezó a mostrar/ocultar este panel también, sin lógica nueva de visibilidad. Se quitó por completo el botón "🎙️ Modo Conversación" que vivía anidado en el header del hilo de un amigo (`index.html`) — pedido explícito de que ya no esté ahí. Como Conversación ya no depende de `activeFriend` (antes tomaba el idioma del amigo abierto), se agregaron DOS selectores de idioma propios dentro de la pestaña (`#conversation-my-lang-grid`/`#conversation-their-lang-grid`, misma plantilla `renderLangPickerGrid()` que "Tu idioma" en Amigos) — "Tú hablas" comparte el mismo idioma guardado que toda la app (`loadChatLanguage()`), "Interlocutor habla" es nuevo y persiste aparte (`CONVERSATION_THEIR_LANG_KEY`). Reescritas `handleConversationUtterance()`/`startConversationListening()`/`openConversationMode()` sin ninguna referencia a `activeFriend`.

**6. Micrófono: antes tragaba errores en silencio.** `recognition.onerror` solo hacía `stopConversationListening()` sin avisar nada — quien tocaba el micrófono con el permiso denegado, o hablaba y no se detectaba voz, no tenía ninguna pista de qué pasó. Ahora distingue el código de error (`not-allowed`/`permission-denied` → "permiso denegado", `no-speech` → "no se detectó voz", cualquier otro → genérico) y lo muestra 4 segundos en `#conversation-mic-status`. La transcripción+traducción+síntesis de voz en sí ya eran reales (Web Speech API + MyMemory + SpeechSynthesis) desde el bloque original — verificado end-to-end con el formulario de texto (que comparte la misma función `handleConversationUtterance()` que usaría el micrófono real): "Hola, como estas?" (ES) → "Hello, how are you?" (EN) renderizado correctamente en ambos paneles.

**Bug real encontrado y corregido durante la propia verificación (no parte del pedido original, pero causado por agregar la 4ta pestaña):** en mobile (375px), la pestaña "🎙️ Conversación" desbordaba por fuera del modal — medí con `getBoundingClientRect()` que su borde derecho quedaba ~50px más allá del borde del contenedor. Causa: `.chat-tab` no tenía `min-width: 0`, y el default `min-width: auto` de un flex item le impide encogerse por debajo del ancho de su contenido — con 3 pestañas nunca se notó, con 4 (una con emoji + palabra larga) sí. Agregado `min-width: 0` + `overflow: hidden; text-overflow: ellipsis` como red de seguridad, y angostado gap/padding/font-size levemente. Reverifiqué con `getBoundingClientRect()` en 375px y 1280px: las 4 pestañas quedan parejas (72px c/u en mobile, 61px c/u en desktop), `scrollWidth === containerWidth` en ambos casos (cero desborde).

**Cache-busting:** `style.css`/`app.js` subidos a `?v=20260809-3` (`index.html`) y `CACHE_NAME` en `sw.js` a la misma versión (partiendo de `v20260809-2`, que ya venía subido en el árbol de trabajo al empezar este bloque). `DB_INIT_FLAG_KEY` subido a `v20260809-2` — necesario para que el `replica identity full` nuevo se aplique incluso en dispositivos que ya hubieran corrido `/api/init-db` con el esquema del Bloque 64.

**Pruebas realizadas (navegador real vía esta sesión, servidor estático local sin runtime de Serverless Functions):**
- `node --check` en `app.js` e `init-db.js`: sin errores de sintaxis.
- Pestaña "Conversación" abre de forma independiente (sin amigo activo), con ambos selectores de idioma pintados y banderas correctas por defecto (ES/EN).
- Formulario de texto (mismo camino que el micrófono real): "Hola, como estas?" tradujo y renderizó correctamente en los dos paneles, sin llamar a Supabase (verificado que sigue sin escribir en `app_friend_messages`, tal como documentado).
- Botón "🎙️ Modo Conversación" confirmado ausente del header del hilo de un amigo (`chat-friend-thread__header` ahora solo tiene back+nombre).
- Crear Escuadrón sin tablas en Supabase: muestra el nuevo mensaje "todavía se está configurando" en vez del genérico anterior.
- Pestaña 🗄️ Base de Datos: botón "Verificar/Reintentar" ejecuta `checkDatabaseStatus()`, maneja el 404 local sin crashear y muestra el error real.
- Desborde de pestañas en mobile (375px) reproducido, corregido y reverificado con `getBoundingClientRect()`; reverificado también en desktop (1280px) sin regresión.
...
## Bloque 67 — Video de intro de vuelta a la ventana del León, blindaje final del Lore, minimalismo del header, y confirmación de protecciones ya existentes

Pedido de 4 puntos "de todo lo conversado": (1) video de intro dentro de la ventana del León (autoplay/muted/playsinline, 2 vueltas exactas, transición a la imagen default); (2) desbloqueo total del Lore (sin candados de nivel); (3) minimalismo de UI (barras laterales/íconos); (4) perfiles independientes + Admin Principal protegido de borrado + chat abierto.

**Nota de contexto — mucho de esto ya estaba hecho por otra sesión paralela.** Antes de tocar nada, `git log`/`git show` revelaron que entre el Bloque 66 y este bloque hubo una cantidad enorme de commits de otra sesión trabajando en el mismo directorio compartido (Mapa de Mundos de Nihongo, módulo de Español, Conversación por Voz con IA, fondos dinámicos, etc. — ninguno documentado en este mismo PROGRESS_LOG.md, que solo refleja el trabajo de las sesiones que sí siguen esta convención). Dos hallazgos concretos de esa investigación:
- El commit `ba477b8` ("Video de intro en la ventana del Leon (2 reproducciones) y desbloqueo total del Modal de Lore") ya había implementado los puntos 1 y 2 — pero el commit `fc9f086`, el MÁS RECIENTE antes de este bloque, deshizo el punto 1 a propósito, reemplazando el video de la ventana del León por un splash a pantalla completa (mecánica distinta: pantalla completa + audio + un toque para entrar). Este bloque revierte esa decisión puntual, sin tocar el resto de lo que aportó `fc9f086`.
- Los commits `a54dab6` ("Protege al Admin Principal del boton Eliminar en Perfiles de Usuario"), `f4abfba` ("Corrige aislamiento real de Sub-Perfiles...") y `441e413` ("...ya no encierra al Operador en 3 opciones") ya cubrían el punto 4 por completo. Se verificó leyendo el código actual (no solo el mensaje del commit, por si algo posterior lo hubiera revertido) que sigue intacto — **no se tocó nada de esto**, para no duplicar ni arriesgar trabajo ya validado.

**1. Video de intro en la ventana del León — `index.html`/`style.css`/`app.js`.** Se quitó `#intro-splash` (overlay de pantalla completa) por completo y se agregó `<video id="avatar-intro-video">` como capa hermana de `#avatar-visual-img` dentro de `#avatarStage`, con los atributos exactos pedidos (`autoplay muted playsinline`). `playAvatarIntroVideo()` (reemplaza a `initIntroSplash()`) reproduce el clip 2 veces exactas — sin `loop`, se reinicia a mano en el primer `ended` para poder contar — y al segundo `ended` desvanece el video y lo saca del DOM, revelando el León/skin activo que ya estaba debajo. Respeta Ajustes → Animaciones desactivadas (se saca del DOM sin intentar nada) y tiene la misma red de seguridad por timeout (5s) que el diseño original, para no dejar la ventana vacía si el archivo no carga.

**2. Blindaje final del Lore — `storyEngine.js`.** El desbloqueo de capítulos (`const desbloqueado = true`) ya estaba hardcodeado desde `ba477b8` y seguía intacto — verificado leyendo el código, no asumido. Se encontró y eliminó un ÚLTIMO camino residual dependiente del nivel: `mostrarVistaCapitulos()` usaba `capituloMasAltoDesbloqueado(lista, nivel)` para decidir con qué capítulo abrir el modal por defecto, y si ningún capítulo cumplía `nivel >= nivel_requerido` (imposible hoy porque el capítulo 1 pide nivel 1, pero no imposible a futuro), caía en `mostrarSinDesbloquear()` — una pantalla vacía sin tabs ni contenido. Se simplificó para abrir siempre el primer capítulo de la lista, sin depender del nivel para nada; se eliminaron `capituloMasAltoDesbloqueado()`, `mostrarSinDesbloquear()` y `MENSAJE_SIN_DESBLOQUEAR` por quedar sin ningún otro uso.

**3. Minimalismo de UI — `index.html`/`style.css`.** Sin poder verificar visualmente en este entorno (el Browser pane no compone capturas acá), se optó por una simplificación concreta y verificable por código en vez de un rediseño especulativo: el header mostraba el nombre del Sub-Perfil y el nombre del Operador en DOS píldoras separadas, cada una con su propio borde/glow (`.btn-profile-switch` dorada + `.hud__profile-name` cian), una al lado de la otra — la misma información de "quién sos" duplicada en dos controles. Se consolidaron en una sola píldora: `#profile-name` (mismo id, cero cambio de lógica en `app.js`) ahora vive DENTRO de `.btn-profile-switch` como línea secundaria más chica. Se limpiaron las referencias muertas a `.hud__profile-name` en la sección de tema claro de Mobile Lite. El resto de los docks laterales/grilla de íconos se revisó y se dejó como está: ya había pasado por varias rondas explícitas de consolidación en sesiones anteriores (fusión de pilares, íconos duplicados eliminados, tarjetas retiradas del App Hub) y no se encontró redundancia real adicional que justificara tocarlo sin ver el resultado en pantalla.

**4. Perfiles independientes / Admin protegido / chat abierto — solo verificación, sin cambios de código.** Confirmado en `app.js`: `isProtectedAdminProfile()` protege tanto al perfil llamado literalmente "Admin" como a cualquiera nombrado con `ADMIN_EMAIL`, con defensa en profundidad (botón oculto en `renderProfileList()` + chequeo repetido dentro de `deleteProfile()`); `PROFILE_SCOPED_KEY_BASES` sigue aislando todos los datos por perfil (state, ledger, hábitos, chat...); el chat del asistente (`generateReply()`) ya no encierra al Operador en opciones fijas, responde abierto a cualquier tema.

**Pruebas realizadas (navegador real, servidor estático local):**
- `node --check` en `app.js` y `storyEngine.js`: sin errores de sintaxis.
- Video: confirmado `autoplay`/`muted`/`readyState:4` al cargar; se simuló el evento `ended` dos veces — la 1ra reinicia `currentTime` y sigue reproduciendo, la 2da saca el `<video>` del DOM del todo y deja `#avatar-visual-img` visible (`display:block`). `#intro-splash` confirmado ausente del DOM.
- Lore: abierto el modal, recorridos los 8 capítulos (hasta "Nv. 70") con un operador de nivel 1 simulado — ninguno mostró el overlay de spoiler/candado, todos con texto real cargado.
- Header: confirmado que `.hud__profile-name` como elemento independiente ya no existe; `#profile-name` vive dentro de `.btn-profile-switch` con el mismo id.
- Modal de Perfiles: abre sin errores; el perfil activo (sin botón eliminar, esperado) se renderiza correctamente.
...
## Bloque 69 — Entorno local (`vercel dev`), cierre del gap multi-admin, y rediseño del Panel de Administrador

Pedido de 3 prioridades: (1) poder correr el proyecto en local con Vercel Functions + disparar `/api/init-db`; (2) blindar el control de acceso al Panel de Administrador; (3) rediseñarlo como dashboard SaaS limpio. Antes de tocar nada se auditó el gate existente: el backend (`resolveAdminSession()` en `api/_utils.js`, usado por los 4 endpoints `admin-*`) ya estaba bien diseñado — degrada a "no admin" ante cualquier fallo, nunca al revés. El bug real estaba en el frontend: `checkAdminSession()`/el submit del login (`app.js`) decidían `isSuperAdmin` comparando el email a mano contra `ADMIN_EMAIL`, **ignorando la tabla `public.admins`** (sistema multi-admin del 2026-08-12) — un Admin agregado por tabla podía loguearse pero nunca veía el botón/panel.

**1. Entorno local.** `miikaeru-web/.env.example` nuevo (plantilla commiteada, mismo patrón que `tools/ai-player/.env.example`) con `SUPABASE_DB_URL` (la única que de verdad hace falta para `vercel dev` — el resto de las funciones `admin-*`/`login-account`/`register-account`/`init-db` dependen de ella) + `SUPABASE_URL`/`SUPABASE_ANON_KEY` documentadas (ya hardcodeadas en el código, no son secretas) + `ANTHROPIC_API_KEY` opcional. `.gitignore` raíz ahora ignora `.env`/`.env.local`/`.env.*.local` en cualquier carpeta (antes solo cubría `tools/ai-player/.env` a mano).

**2. Seguridad — 3 cambios, todos alrededor del mismo gap multi-admin.**
- `api/admin-verify.js` (nuevo): endpoint GET liviano que reusa `resolveAdminSession()` — única fuente de verdad server-side sobre "¿esta sesión es Admin?".
- `app.js`: `checkAdminSession()` y el submit del login ahora llaman a `/api/admin-verify` en vez de comparar el email a mano — un Admin agregado por tabla ve el panel correctamente. Fail-closed si el fetch falla (offline, o local sin `vercel dev`). `openAdminPanel()` ahora también avisa con un mensaje de sistema si se invoca sin permiso (antes no hacía nada, silencioso) — defensa en profundidad ante alguien forzando la llamada desde la consola.
- `api/init-db.js`: se encontró que el gap se repetía un nivel más abajo — la policy RLS `"admin can read feedback"` (Bandeja de Sugerencias) solo reconocía `auth.jwt()->>'email' = ADMIN_EMAIL`, nunca `public.admins`. Como `admins` tiene RLS sin ninguna policy, un `exists(select ... from public.admins)` normal dentro de la policy de `feedback` se hubiera topado con esa misma RLS y no habría servido de nada. Se agregó `public.is_admin(check_email)`, función `SECURITY DEFINER` (corre con el privilegio de la conexión de `SUPABASE_DB_URL`, que sí puede leer `admins` sin RLS de por medio) que ambas policies de `feedback` (`select`/`update`) ahora usan — idempotente (`create or replace`), se aplica solo con volver a correr `/api/init-db`.

**3. Rediseño del Panel de Administrador — `index.html`/`app.js`/`style.css`.** Mismo modal/IDs de siempre (ninguna lógica de datos existente se reescribió), reestructurado como sidebar + contenido (antes: fila de 9 pestañas horizontales). 4 pestañas nuevas al principio del sidebar:
- **Resumen Ejecutivo** (pestaña por defecto): 4 stat cards (Operadores/Relaciones/Transacciones/Sugerencias Pendientes) — no dispara fetches propios, `syncOverviewStats()` refleja los números que ya cargan Métricas/Transacciones/Inspector al terminar cada una.
- **Salud del Sistema**: grilla de 13 chips (uno por tabla de `/api/init-db`) en vez del JSON crudo (que se mudó, sin cambios, a "Base de Datos (técnico)"). `renderSystemHealth()` distingue 3 estados por chip — neutro antes de verificar, ✅/❌ con resultado real, y **❓ si `/api/init-db` fue directamente inalcanzable** (bug real encontrado en la verificación en navegador: la primera versión mostraba falso ✅ en las 13 tablas ante un fallo total de red/parseo, porque `failedStatements` vacío se interpretaba como "todo bien" en vez de "no se sabe").
- **Actividad en Tiempo Real**: se suscribe a Supabase Realtime (`postgres_changes` INSERT) en `transactions` y `feedback` solo mientras esta pestaña está abierta (mismo patrón que `wireFriendshipsRealtime()`). Deliberadamente NO incluye `public.users` (altas de Operador): esa tabla no tiene ninguna policy RLS de lectura, y el payload de Realtime trae la fila COMPLETA — incluida `password` (hash) — algo que `admin-list-users.js` evita a propósito ni pidiéndola en su `SELECT`.
- **Usuarios y Progresión**: mismo tab de siempre, sin cambios de lógica, solo agrupado bajo el nuevo layout.

Las 8 pestañas restantes (Transacciones/Inspector/Automatización/Métricas detalle/Fotos/Fondos/Base de Datos técnico/Administradores) quedan agrupadas bajo "Configuración" en el mismo sidebar, mismo look, cero cambios de lógica. CSS nuevo (bloque delimitado al final de `style.css`) reusa los tokens `--neon-*`/`--bg-*`/`--glass-*` ya existentes — acentos de color reservados a estado/énfasis (cada stat card con su propio color de borde superior), no todo el panel a color. `style.css?v=`/`app.js?v=` en `index.html` y `CACHE_NAME` en `sw.js` subidos a `20260818-1`.

**Pruebas realizadas (navegador real, servidor estático local vía `tools/dev-server.js` — sin Vercel Functions/Supabase reales disponibles en este entorno):**
- `node --check` en `app.js`/`api/init-db.js`/`api/admin-verify.js`: sin errores de sintaxis.
- Las 12 pestañas del sidebar (incluidas las 4 nuevas) se probaron una por una vía click programático: todas togglean su pane correctamente, 0 errores nuevos en consola.
- Layout confirmado por `getBoundingClientRect()`: sidebar 200px fijo, 12 botones de navegación, 4 stat cards con `border-top-color` correcto (cian/verde/dorado/magenta) para cada métrica.
- Salud del Sistema: confirmados los 3 estados del chip — se detectó y corrigió en el momento el bug del falso-✅ descrito arriba (verificado de nuevo tras el fix: ahora sí muestra ❓ en las 13 tablas ante un fallo total de red, no ✅).
- No se pudo probar el flujo de login real ni las llamadas a `/api/*` (requieren `vercel dev` + `SUPABASE_DB_URL` real, fuera del alcance de este entorno) — el usuario debe correr `vercel dev` local para esa verificación end-to-end (instrucciones entregadas en el chat).

**Pendiente/nota honesta:** el Modo Local Autónomo de Métricas (`renderLocalOnlyMetrics()`, sin Supabase disponible) sigue sin tocarse — el Resumen Ejecutivo hereda ese mismo comportamiento sin cambios. La Actividad en Tiempo Real solo cubre `transactions`/`feedback` a propósito (ver nota de seguridad arriba); si más adelante se quiere sumar altas de Operador ahí, hace falta antes decidir una policy RLS específica para `public.users` que NO exponga `password`, no es un cambio de una línea.
...
## Bloque 70 — Corrección: el Bloque 69 no reflejaba el Plan Estratégico real

El usuario marcó el Bloque 69 como un error directo: "es una copia barata de la aplicación Miikaeru", "no tiene nada que ver con lo que te pedí". Tenía razón en dos cosas concretas — (1) reusé a propósito los mismos componentes visuales del resto de Miikaeru (`.admin-panel__table`/`.inspector-stat`/`.admin-stat-card` con bordes neón duros) en vez de darle identidad propia a este dashboard; (2) el contenido del "Resumen Ejecutivo" (4 tarjetas de conteo genéricas) no tenía ninguna relación con el Plan Estratégico real de 3 Capas / hitos de crecimiento / monetización progresiva que el usuario compartió recién en este bloque (no estaba disponible antes — ni en la conversación ni en memoria).

**1. Lenguaje visual nuevo — bloque `.miika-*` en `style.css`.** Tarjetas de vidrio con degradés suaves (`linear-gradient` translúcido por color de acento, esquinas de 20px, sin `box-shadow` neón duro), inspiradas en las 2 imágenes de referencia que compartió el usuario (dashboard de métricas tipo SaaS + mockup de asistente IA oscuro), adaptadas a la paleta oscura ya establecida de Miikaeru en vez de copiar el fondo claro de la primera referencia. Deliberadamente separado de `.admin-panel__*`/`.inspector-stat`/`.admin-stat-card` (que se dejaron intactos: siguen sirviendo bien a las pestañas operativas de Transacciones/Usuarios/Fotos/etc., que no eran el problema).

**2. Pestaña "Crecimiento" (reemplaza al "Resumen Ejecutivo" del Bloque 69) — `app.js`/`index.html`.** `GROWTH_MILESTONES`: los 7 hitos EXACTOS del Plan Estratégico (Lanzamiento Oficial → 100 → 1.000 → 5.000 → 20.000 → 100.000 → 1.000.000 de usuarios), cada uno con la nota específica del plan (ej.: "5.000 usuarios: Optimización de consultas SQL en Supabase para el ranking social y comunidades"). `renderGrowthMilestones(count)` compara el total REAL de Operadores (mismo dato que ya trae `/api/admin-metrics`, sin inventar ninguna cifra) contra esos 7 umbrales: anillo de progreso (`conic-gradient` puro, sin SVG ni librería) mostrando % hacia el próximo hito, etapa actual, y una lista tipo timeline con los 7 hitos marcados alcanzado/actual/próximo. Probado con 0 operadores (Lanzamiento, 0%) y simulando 3.400 (etapa "1.000 usuarios", 60% hacia 5.000, fila `current` correcta) — la lógica de umbrales quedó verificada, no solo el layout.

**3. Pestaña "Ecosistema (3 Capas)" (NUEVA) — contenido estático.** 3 tarjetas con la Sección 2 del plan tal cual. El estado de cada capa se verificó por código, no se inventó: se lanzó un subagente Explore a buscar cualquier rastro de "páginas personales de citas" (booking, appointment, Stripe/PayPal/MercadoPago, tabla de servicios) en todo el repo — cero resultados reales (los únicos hits eran "Método de Pago" del ledger interno de Finanzas y vocabulario japonés de flashcards, sin relación). Capa 1 queda marcada honestamente "No implementado aún", Capa 2 "Estás acá", Capa 3 "En producción".

**4. Pestaña "Monetización" (NUEVA) — contenido estático.** 3 tarjetas con la Sección 4 del plan (Fase 1/2/3), todas marcadas "Planeado" — se confirmó por el mismo motivo que Ecosistema que ni las páginas de citas ni el sistema de donaciones tienen código de cobro todavía, así que mostrar cifras de ingresos acá habría sido inventarlas.

**5. Salud del Sistema / Actividad en Tiempo Real (del Bloque 69) — reskin, sin tocar lógica.** Los chips de tabla y las entradas del feed de actividad pasaron del borde neón duro al mismo lenguaje `.miika-*` (degradé suave, esquinas de 14px) para que todo el dashboard se sienta como un solo sistema, no un parche encima de otro.

**Versionado:** `style.css?v=`/`app.js?v=` → `20260818-2`, `CACHE_NAME` en `sw.js` → mismo sufijo.

**Pruebas realizadas (navegador real, contra `vercel dev` local corriendo en `http://localhost:3000` — no contra el servidor estático esta vez):** `node --check app.js` sin errores; las 14 pestañas del sidebar togglean correctamente con 0 errores de consola; hito de crecimiento verificado con 2 valores de operadores distintos (0 → Lanzamiento/0%; 3.400 → etapa "1.000 usuarios"/60%/fila `current` correcta); 3 tarjetas de Ecosistema + 3 de Monetización confirmadas en el DOM. `SUPABASE_DB_URL` seguía sin completarse en `.env.local` del lado del usuario al momento de esta verificación (variable "Sensitive" en Vercel, ver nota del Bloque 69) — no bloqueó nada de lo de arriba porque estas 2 pestañas nuevas son 100% estáticas y Crecimiento reusa datos ya cacheados en el DOM, pero las llamadas reales a `/api/admin-metrics` con datos de producción siguen sin poder probarse en este entorno.

**Corrección 70b — el usuario, con razón, dijo que seguía sin verse como la referencia.** Volvió a mandar la imagen de referencia ("Hello, Alex", tarjetas oscuras planas + acentos de color solo en anillo/números). Comparando: mis tarjetas `.miika-*` tenían un degradé de COLOR de fondo (`linear-gradient` con el tono de cada acento lavado sobre toda la tarjeta) — la referencia usa tarjetas prácticamente negras y planas, con el color reservado al anillo/ícono/número. Se corrigió: fondo de tarjeta → `rgba(8,11,18,0.55)` plano para todas (tiles/hero/layer/phase/health-chip/activity-entry); el punto de color de cada tile pasó de "toda la tarjeta lavada" a un puntito de 8px arriba a la derecha; el anillo de progreso pasó de un solo color (cian) a un `conic-gradient` de 3 colores (`--neon-purple` → `--neon-magenta` → `--neon-gold`), igual que el gauge "76 Score" de la referencia; la lista de hitos pasó de timeline-con-círculos-conectados a filas con barra de acento a la izquierda (mismo patrón que "09:00 Team Stand-up" en la referencia).

**Bug real encontrado en el proceso — comentarios CSS con `*/` literal dentro del texto.** Al verificar el resultado en el navegador, `border-radius` de las tarjetas nuevas daba `0px` en vez de `20px` pese a que la regla se veía bien en el archivo. Se armó un script Node que recorre `style.css` carácter por carácter llevando el estado "¿estoy dentro de un comentario?" (en vez de solo contar `/*` vs `*/`, que no detecta el problema) — encontró que el propio comentario de cabecera de este bloque escribía `(--neon-*/--bg-*/--glass-*)`, y el `*` seguido de `/` ahí cierra el comentario CSS a mitad de frase sin querer. Todo lo que sigue hasta el `*/` real de verdad queda como CSS suelto no comentado, que el navegador no logra parsear como selector válido — y descarta la regla completa que viene después. Eso se comió `.miika-dash { --miika-radius: 20px; }` (de este bloque) y, más grave, la regla `.modal--admin-panel.admin-dash { max-width: 980px; width: 92vw; ... }` del Bloque 69 (el mismo bug, mismo patrón `.admin-panel__*/.inspector-stat`, escrito ANTES en ese bloque) — es decir, **el modal del Panel de Administrador nunca se ensanchó de verdad**, se quedó en los 720px originales desde el Bloque 69, lo cual seguramente contribuyó a que se sintiera "igual que antes". Ambos comentarios se reescribieron sin `*/` literal (comas en vez de barras). Verificado tras el fix: `modalMaxWidth` resuelve a `980px`, `border-radius` de `.miika-tile` resuelve a `20px`, el anillo resuelve al `conic-gradient` de 3 colores esperado.

De paso se encontró un TERCER caso del mismo bug, preexistente y sin relación con este trabajo (Módulo de Idiomas, línea ~7615 — `.jp-vocab-*/.jp-grammar-*/.jp-quiz-*/`), que probablemente rompe el layout de `.lang-stats-row`. No se tocó: se dejó como tarea aparte (`spawn_task`, fuera del alcance de este bloque) para no mezclar un fix no verificado de otro módulo con este.

**Versionado:** `style.css?v=`/`app.js?v=` → `20260818-4`, `CACHE_NAME` en `sw.js` → mismo sufijo.

---

## Bloque 71 — El malentendido real: no era una pestaña del Panel de Administrador, era una app aparte

Después del Bloque 70b el usuario insistió una tercera vez con la misma imagen de referencia, ya visiblemente frustrado ("por qué no entendés, qué tengo que hacer para que entiendas"). Ahí se preguntó explícitamente qué pantalla quería decir — la respuesta cambió todo: *"esto no tiene nada que ver con el León... esta es una app diferente, lo único que tiene que sacar de la aplicación Miikaeru son las métricas de los usuarios y donaciones"*. Los Bloques 69/70/70b habían estado construyendo todo esto como pestañas DENTRO del `#admin-panel-modal` de la app principal — el propio usuario había dicho "3 Capas" en su Plan Estratégico desde el principio (Bloque 69), pero se interpretó como 3 secciones de un mismo panel en vez de 3 productos separados del ecosistema. Confirmado con 2 preguntas concretas antes de tocar código: (1) misma repo, carpeta nueva — no un repo/proyecto de Vercel aparte; (2) esta app nueva es SOLO lectura de métricas de usuarios y donaciones — todo lo demás (editar Nivel/XP, Transacciones, Fotos, Fondos, Automatización, Bandeja de Sugerencias) se queda exactamente donde estaba, sin tocar.

**1. Revert del Panel de Administrador a como estaba ANTES del Bloque 69.** `index.html`/`style.css`/`app.js` restaurados byte a byte desde el commit `12d0ee2` (el HEAD justo antes de empezar todo esto) vía `git show 12d0ee2:archivo > archivo` — no un revert manual campo por campo, para no arrastrar ningún resto de las 3 rondas anteriores. Sobre esa base limpia se reaplicó SOLO el fix de seguridad real que sigue siendo válido (el gap multi-admin, ver Bloque 69): `verifyAdminViaApi()` en `checkAdminSession()`/el submit del login (llama a `/api/admin-verify` en vez de comparar el email a mano), y el mensaje de "no autorizado" en `openAdminPanel()`. `api/admin-verify.js` y el `is_admin()` SECURITY DEFINER de `init-db.js` no se tocaron — son backend, nunca dependieron de nada visual. Verificado: las 9 pestañas originales (Transacciones primero, sin sidebar, sin `.miika-dash`) funcionan sin errores de consola.

**2. App nueva — `miikaeru-web/dashboard/` (index.html + style.css + app.js propios).** Cero dependencia del `app.js` de 1.7MB ni del `style.css` de 12k+ líneas de la app principal — HTML/CSS/JS escritos de cero, sirviéndose en `/dashboard/` sin configuración extra de `vercel.json` (el `outputDirectory: "."` ya lo resuelve solo). Reusa lo único que YA es compartido entre cualquier frontend de este proyecto: las mismas Serverless Functions (`/api/admin-verify`, `/api/admin-metrics`) y el mismo proyecto de Supabase Auth (mismas `SUPABASE_URL`/`SUPABASE_ANON_KEY` públicas). Login propio (email+contraseña de Supabase Auth) con el mismo criterio de seguridad que la app principal — nunca decide "es Admin" comparando el email en el cliente, siempre confirma contra `/api/admin-verify`. Pantalla de inicio: anillo de crecimiento (mismo `conic-gradient` de 3 colores del Bloque 70b) + lista de los 7 hitos del Plan Estratégico + tarjetas de Operadores/Relaciones (dato real de `/api/admin-metrics`) y Donaciones — esta última mostrada honestamente en S/ 0 con la nota "Aún no implementado en la app principal" (se verificó otra vez que no existe ninguna tabla/endpoint de donaciones, no se inventó el dato).

**Pruebas realizadas (navegador real, contra `vercel dev` local en `http://localhost:3000/dashboard/`):**
- `node --check` en `app.js` (principal) y `dashboard/app.js`: sin errores.
- Panel de Administrador viejo: confirmado por DOM que las 9 pestañas son exactamente las originales, "transactions" activa por default, sin rastro de `.admin-dash__sidebar`/`.miika-dash`; las 9 togglean sin errores de consola.
- `dashboard/`: carga en `/dashboard/` con `style.css?v=1`/`app.js?v=1` en 200 OK, sin errores propios en consola (los 500/errores que aparecían en la misma sesión de navegador eran ruido de una carga anterior de la app principal en la misma pestaña, confirmado revisando cada request por separado). Estado inicial correcto: pantalla de login visible, dashboard oculto. Forzando el estado autenticado a mano: anillo con el `conic-gradient` de 3 colores esperado, tarjetas con `border-radius:20px` y fondo plano `rgb(13,16,22)`, fila de hito actual con `border-left` dorado — mismo lenguaje visual ya validado en el Bloque 70b, ahora en una página realmente independiente.
- Se aplicó el mismo chequeo anti-`*/`-literal del Bloque 70b al `dashboard/style.css` nuevo antes de darlo por bueno: 0 comentarios rotos.

**Versionado:** `style.css?v=`/`app.js?v=` de la app principal → `20260818-5` (vuelven a existir sin las clases `.admin-dash`/`.miika-*`, que se mudaron a `dashboard/style.css`). `dashboard/style.css`/`dashboard/app.js` arrancan en `v=1`, versionado independiente del resto del sitio.

---

## Bloque 72 — Bug real de Kanji N5 (candado invisible que no dejaba entrar) + bug real de "Maximizar" en las ventanas flotantes (botón de restaurar inalcanzable)

Dos reportes del usuario en el mismo bloque: (1) "en Kanji N5 no hay ningún mecanismo de aprendizaje... o quizás ya existe y hay un bug que no me deja entrar, ya te había pedido esto antes y no se solucionó" — con capturas del Mapa de Mundos de Kanji N5 mostrando los nodos "7"/"8" sin candado visible; (2) más tarde, mientras el usuario se iba a dormir, pidió arreglar de paso que "cuando pongo maximizar [en una ventana flotante] ya no se puede hacer más, no poder configurarlo... con la flechita grande/pequeño, y no me da acceso a la app" — con una captura del panel del León maximizado.

**1. Kanji N5 — causa raíz real encontrada por lectura de código (`app.js`).** `JP_WORLDS["kanji-n5"].buildContentStops()` hardcodeaba `locked: false` en TODOS los nodos del mapa, sin importar el nivel real del Operador — pero `getKanaList("kanji")` (la función que de verdad arma la lista de práctica) SÍ aplica un candado real por kanji individual (`jpKanjiUnlockLevel(i) = 30 + Math.floor(i/2)`, Niveles 30-83). Como un Operador típico (Nivel 1-15) no tiene NINGÚN kanji desbloqueado todavía, `startJpKanjiChunkPractice()` terminaba llamando a `startJpPractice([])` con la lista vacía — y `startJpPractice()` literalmente dice `if (!kanaList.length) return;`: cero feedback, cero mensaje, el nodo "no hacía nada" al tocarlo. A diferencia de Hiragana/Katakana (100% desbloqueados desde el inicio, `jpKanaRowUnlockLevel()` siempre devuelve 1), Kanji N5 sí depende del nivel — pero el mapa nunca reflejaba ese candado real. Corregido: cada chunk ahora calcula su `requiredLevel`/`locked` real y usa el MISMO patrón `locked`/`onLockedClick` + `notifyJpLocked()` que ya usan otros nodos bloqueados del mapa (candado 🔒 visible, mensaje de nivel real al tocarlo) — cero código nuevo, solo conectar el chunk de Kanji al mecanismo de candado que ya existía para el resto del Mapa de Mundos.

**2. Ventanas flotantes — "Maximizar" con el botón de Restaurar geométricamente inalcanzable.** Diagnóstico reproducido EN VIVO (no solo por lectura de código) simulando una secuencia real de puntero (`pointerdown`→`pointerup`→`click`, no `.click()` sintético — ese sí "funciona" porque salta el hit-testing real) sobre las coordenadas exactas del botón: `document.elementFromPoint(x, y)` devolvía `<header class="hud">`, no el botón. Causa raíz real: `.floating-window--maximized` usaba `inset:14px` parejo en los 4 lados — el borde superior (con los botones de minimizar/maximizar) quedaba a solo 14px del techo del viewport, DEBAJO del header fijo real (que en anchos intermedios como 1280px mide 153px de alto, 2 filas de stats). El header (`position:relative` + `z-index:2` propio, hermano de `<main>`) le gana el apilamiento a CUALQUIER z-index interno de un panel anidado varios niveles adentro de `<main>` (que no tiene su propio z-index) — el botón se veía perfectamente a través del `backdrop-filter:blur` del header, pero un click real siempre caía sobre el header, invisible e inmóvil. Arreglado con dos partes:
   - `floatingWindow.js`: nueva opción `maximizeTop` (análoga a `minTop`/`spawnMinTop`, con el mismo patrón de función-evaluada-en-vivo) + `_getMaximizeTop()`; `toggleMaximize()` ahora fija `this.el.style.top` explícito al entrar en modo maximizado, en vez de confiar solo en CSS.
   - `app.js`: `floatingWindowMaximizeTop()` mide en vivo `.hud-banner`/`header.hud` (a diferencia del tope general de arrastre, que evita medir en vivo a propósito por un bug histórico de 270px+ — acá SÍ vale la pena: un tope "demasiado profundo" en un caso extremo solo resta algo de alto útil, muy distinto al daño real de un tope insuficiente, que rompe el click por completo). Conectado a las 4 ventanas flotantes (León, Chat, ambos docks).
   - **Segundo bug real, encontrado DURANTE la verificación del primer fix:** la regla CSS de respaldo tenía `top: ... !important` — con `!important` en una hoja de estilos, NADA que un script ponga como `element.style.top` puede ganarle nunca, sin importar qué tan buena sea la medición en vivo. El primer intento de fix parecía "no hacer nada" (`getComputedStyle` seguía devolviendo el valor fijo) hasta encontrar esto. Se sacó el `!important` de `top` específicamente (se mantiene en los otros 3 lados, que ningún script toca).

**Pruebas realizadas (navegador real, servidor estático local):**
- `node --check` en `app.js`/`floatingWindow.js`: sin errores.
- Kanji N5: verificado por lectura que `notifyJpLocked`/`isJpLevelUnlocked`/`jpKanjiUnlockLevel` son alcanzables desde el nuevo código (mismo scope que el resto del módulo Nihongo) — sin poder llegar a la vista exacta del mapa en esta sesión de prueba (onboarding distinto), la sintaxis y el patrón reusado (idéntico al de `jpConversationExamStop`, ya en producción) dan alta confianza.
- Maximizar/Restaurar: reproducido el bug ANTES del fix (`elementFromPoint` devolvía `HEADER.hud`, no el botón); después del fix (con caché realmente invalidada — encontrado que el primer re-test seguía viendo el bug por caché HTTP del navegador, no por el `!important`, hasta subir `?v=`), una secuencia de puntero real sobre las coordenadas exactas del botón: `hitElWasBtn: true`, `isMaximized` pasa de `true` a `false` — restaurado con éxito. Mismo resultado verificado también en el panel de Chat (`hitElIsBtn: true` tras maximizar).
- 0 errores de JavaScript nuevos en consola en todo el flujo.

**Nota honesta:** el usuario pidió esto último mientras se iba a dormir, autorizando de antemano "aceptar todo lo recomendado" — se ejecutó de punta a punta sin pausas intermedias, incluyendo push a producción.

**Pendiente/nota honesta:** `SUPABASE_DB_URL` en `.env.local` seguía en el placeholder `[SENSITIVE]` de Vercel al momento de esta verificación (ver Bloque 69) — todo lo de arriba se probó forzando el estado autenticado a mano en el DOM, no con un login real ni datos reales de producción. El usuario todavía necesita completar ese valor a mano para poder probar el flujo de login + métricas reales de punta a punta.

## Bloque 73 — Cursor táctico futurista + Vista Hero opcional (inspirados en el prototipo externo "MetricasAI")

El usuario pidió una "fusión y migración total" entre un prototipo externo standalone llamado MetricasAI (`Desktop/MiSuiteDeApps/MetricasAI/index.html`, 1166 líneas, Tailwind CDN, sin backend real) y Miikaeru_Sys (30.000+ líneas entre `index.html`/`app.js`, producción real, 72 Bloques de historial). Antes de tocar nada se leyó el prototipo completo para no migrar una arquitectura inventada. Se le explicó al usuario el riesgo real: reescribir por completo `index.html` reemplazando el HUD entero por la navegación de scroll-snap de MetricasAI, en una sola pasada autónoma y sin poder ver el resultado (las capturas de pantalla no habían funcionado en ninguna sesión previa de esta conversación), podía romper meses de trabajo de forma indetectable — justo lo contrario de la condición que el propio usuario puso ("que el código no se rompa y sea estable"). El usuario aceptó ir "a la segura": nada de reemplazar el HUD, solo (1) el cursor SVG y (2) la Vista Hero como capa nueva, opcional y 100% reversible, para probarla e iterar en vivo.

**1. Cursor táctico futurista (`style.css`).** Se portó la retícula SVG cian de MetricasAI tal cual (mismo `data:image/svg+xml` incrustado, sin archivos externos), aplicada en `html, body` con especificidad baja a propósito: los cursores funcionales que ya existían (`grab`/`grabbing` de arrastre, `ns-resize`/`ew-resize`/etc. de las asas de resize, `pointer` de botones) siguen ganando la cascada sin tocarlos, porque viven en selectores de clase más específicos. Cero riesgo, cero lógica tocada.

**2. Vista Hero — overlay opcional, NO un reemplazo del HUD.** Nuevo botón `#hero-view-toggle-btn` (🌌) en el header, junto a Ajustes. Al abrirlo, un overlay de pantalla completa (`#hero-view`) muestra: el holograma del avatar (mismo asset `mikaeru_idle_chakras.png` ya cacheado por el Service Worker, con `mix-blend-mode:screen` + máscara radial + animación de pulso/flotación, igual que el Hero de MetricasAI), debajo una lista vertical de 10 tarjetas (una por cada Pilar/App real: Finanzas, Templo, Aprendizaje, Boss Fight, Calendario, Karaoke, Chat, Wishlist, Territorio, Héroe), y un buscador fijo abajo (`#hero-view-command-form`) que hace `scrollIntoView({behavior:'smooth'})` + un flash de resalte sobre la tarjeta que coincide con lo escrito — exactamente el pedido original ("al pedir una herramienta, que la interfaz haga scroll suave hacia ella"), sin auto-abrir el panel: el usuario decide si clickea después de verla.

**Cero lógica duplicada, a propósito.** Cada tarjeta usa el mismo mecanismo `data-forward` que ya usa en producción el "Tablero de Comando" móvil (`#cyber-grid`, ver Bloque 68): el selector del botón REAL (`.pillar-btn[data-pillar="finanzas"]`, `#chat-open-btn`, etc.) vive en el `data-forward`, y clickear la tarjeta simplemente dispara `.click()` sobre ese botón real — no se reimplementó ninguna función de apertura de panel, así que candados, estado de colaborador activo, etc. siguen gobernados 100% por el código de siempre. Se cierra con el botón "✕ Volver al HUD", con Escape, o automáticamente al reenviar el click a una tarjeta — y al cerrarse, el HUD queda exactamente como estaba, sin recargar ni perder estado.

Colocado como sibling de `<header>`/`<main>` a nivel de `<body>` (mismo patrón que `.modal-overlay`, `z-index:60`) a propósito: evita repetir el bug de stacking-context del Bloque 72 (un descendiente de `<main>`, que no tiene z-index propio, no le puede ganar el apilamiento a un `<header>` hermano con su propio z-index). Toda la paleta usa las variables ya existentes (`--glass-bg`, `--glass-border`, `--neon-cyan`, `--bg-base`, etc.), así que hereda automáticamente el tema "Mobile Lite" claro en pantallas ≤767px y el Cyberpunk oscuro completo en desktop/tablet, igual que el resto de la app — no se hardcodeó ningún color nuevo.

**Pruebas realizadas (navegador real, servidor estático local, capturas de pantalla SÍ funcionaron en esta sesión):**
- Cursor: `getComputedStyle(document.body).cursor` empieza con `url(...)` tras el reload — confirmado.
- Apertura real vía `#hero-view-toggle-btn.click()` (no solo forzado por JS): `hero-view.hidden` pasa a `false`. Cierre real vía `#hero-view-exit-btn.click()` y vía tecla Escape: ambos devuelven `hero-view.hidden === true`.
- Las 10 tarjetas confirmadas por texto/`data-forward` vía DOM — ninguna perdida.
- Click en la tarjeta "Finanzas" real: `hero-view` se cierra (`hidden:true`) Y `#pillar-modal` se abre (`hidden:false`) — captura de pantalla confirma el modal de Finanzas real renderizado con el HUD completo intacto detrás (docks, avatar, stats) — cero regresión visual.
- Buscador "calendario" + submit: la tarjeta de Calendario recibe la clase `hero-card--flash` y su `getBoundingClientRect()` cae dentro del viewport visible — scroll + resalte confirmados.
- Sin errores nuevos en consola (los únicos 404 son `/api/init-db` y llamadas a Supabase, esperado: no hay backend/Vercel functions corriendo en el servidor estático local).
- `?v=` de `style.css`/`app.js` subido a `20260821-1`, `CACHE_NAME` de `sw.js` subido a `miikaeru-cache-v20260821-1` (misma disciplina de siempre, para que usuarios que vuelven vean el cambio).

**Pendiente/próximo paso (decisión explícita del usuario, no un olvido):** esto es intencionalmente solo el primer incremento verificado. Lo que MetricasAI pedía originalmente como reemplazo TOTAL del HUD (scroll-snap como navegación principal, avatar SOLO en el Hero) sigue sin implementarse — quedó como una capa nueva y opcional en paralelo, no como reemplazo, hasta poder iterar en vivo con el usuario paso a paso sobre esta base ya funcionando.

## Bloque 74 — Pulido visual de la Vista Hero: avatar imponente, tarjetas de vidrio esmerilado, transición fluida

Segundo incremento sobre el Bloque 73, pedido explícito tras probar la Vista Hero en vivo: (1) avatar más grande/imponente sin caja, (2) tarjetas con "vidrio esmerilado avanzado" (backdrop-blur + borde fino cian), (3) transición de apertura/cierre fluida "tipo sistema operativo de alta gama". Los tres cambios son puramente visuales/de interacción sobre la MISMA Vista Hero del Bloque 73 — ningún panel real, ninguna lógica de apertura (`data-forward`), ningún otro archivo tocado.

**1. Avatar imponente.** `width` de `.hero-view__avatar` subido de `min(58vw, 420px)` a `min(78vw, 720px)` (móvil: `min(72vw,340px)` → `min(90vw,460px)`), `.hero-view__stage` más alto (`min(66vh,560px)` → `min(88vh,820px)`) para que tenga aire real arriba, `drop-shadow` del aura de 70px a 100px y la amplitud del flotado de 16px a 22px para que la presencia "imponente" no se sienta desproporcionada al nuevo tamaño. `mix-blend-mode:screen` + la máscara radial (la fusión sin bordes) y la animación de pulso/flotación se mantuvieron sin tocar — solo cambiaron números de tamaño/posición, no el mecanismo.

**2. Tarjetas — vidrio esmerilado avanzado.** `.hero-card` ahora tiene `backdrop-filter: blur(14px) saturate(160%)` real (antes solo tenía un fondo semitransparente plano, sin blur de lo que hay detrás) + un borde cian fino permanente (`rgba(0,240,255,0.22)`, antes solo aparecía cian al hacer hover) + un highlight interior de 1px (`inset 0 1px 0 rgba(255,255,255,.05)`) que simula el bisel de vidrio. En hover, además del glow cian ya existente, ahora también tiñe el fondo levemente (`rgba(0,240,255,.06)`).

**3. Transición de apertura/cierre.** Antes `#hero-view` se mostraba/ocultaba con el atributo `hidden` (salto instantáneo, sin animación posible porque `[hidden]{display:none!important}` no transiciona). Ahora: `.hero-view` por defecto vive en `opacity:0; visibility:hidden; transform:scale(.97); pointer-events:none` (invisible y no-interactivo sin usar `display:none`, así que SÍ puede transicionar), y una nueva clase `.hero-view--open` la lleva a `opacity:1; scale(1); pointer-events:auto` con un `transition` de 0.32s (`cubic-bezier(.4,0,.2,1)`, la misma curva "material" que usan los sistemas operativos modernos para fade+zoom). `openHeroView()`/`closeHeroView()` en `app.js` ahora manejan el timing: al abrir, doble `requestAnimationFrame` antes de agregar la clase (si no, el navegador pinta el estado inicial y el final en el mismo frame y no se ve nada animar); al cerrar, se quita la clase primero (dispara el fade de salida) y el atributo `hidden` real recién se pone 320ms después por `setTimeout` — mismo número que la duración del CSS, documentado en ambos archivos para que no se desincronicen. Se agregó también `@media (prefers-reduced-motion: reduce)` que apaga el pulso/flotación del avatar y deja la apertura/cierre casi instantánea, sin romper nada (es 100% decorativo).

**Pruebas realizadas (navegador real, con capturas de pantalla):**
- `?v=` de `style.css`/`app.js` subido a `20260821-2`, `CACHE_NAME` de `sw.js` a `miikaeru-cache-v20260821-2` (el primer re-test después de este cambio sirvió CSS viejo por caché HTTP hasta subir la versión — mismo síntoma ya documentado en el Bloque 72, resuelto igual).
- Apertura real vía `#hero-view-toggle-btn.click()`: opacity pasa de `"0"`/sin clase justo después del click a `"1"`/`hero-view--open` tras ~1s — transición confirmada, no un salto.
- `avatar.getBoundingClientRect().width` en viewport 1280px: `720px` (antes `420px`) — confirmado el nuevo tamaño imponente.
- `getComputedStyle(card).backdropFilter`: `"blur(14px) saturate(1.6)"`, `borderColor: "rgba(0, 240, 255, 0.22)"` — vidrio esmerilado + borde cian confirmados.
- Click en la tarjeta "Templo" real (después del pulido visual, para confirmar que `data-forward` sigue intacto): `#hero-view` se cierra (`hidden:true` tras el fade) Y `#templo-modal` se abre (`hidden:false`) — cero regresión funcional.
- 0 errores nuevos en consola (mismos 404 esperados de `/api/init-db`, sin backend local).